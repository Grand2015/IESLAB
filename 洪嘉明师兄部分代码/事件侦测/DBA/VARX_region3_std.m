...
%% VARX 模型建立
region1 = [pressure5m(:,2),pressure5m(:,3),pressure5m(:,5),pressure5m(:,8),pressure5m(:,10)];
...
k = 4;
disc_std4 =[ disc_day1(:,k),disc_day2(:,k),disc_day3(:,k),disc_day4(:,k),disc_day5(:,k),disc_day6(:,k),disc_day7(:,k),disc_day8(:,k),disc_day9(:,k)]'; 
k = 5;
disc_std5 =[ disc_day1(:,k),disc_day2(:,k),disc_day3(:,k),disc_day4(:,k),disc_day5(:,k),disc_day6(:,k),disc_day7(:,k),disc_day8(:,k),disc_day9(:,k)]';


std1 = std(disc_std1,1)';
std2 = std(disc_std2,1)';
std3 = std(disc_std3,1)';
std4 = std(disc_std4,1)';
std5 = std(disc_std5,1)';
std  = [std1,std2,std3,std4,std5];


%% 确定上下边界值
for i = 1:5
    disc_down(:,i) = disc_mean(:,i)-2.5*std(:,i);
end

for i = 1:5
    disc_up(:,i) = disc_mean(:,i)+2.5*std(:,i);
end
